 ...
 */
wp_register_script( 'manage-survey-script', plugins_url() . '/mf-survey/scripts/manage-survey-script.js', 'jquery' );
...
<div class="wrap">
	<table width="100%">
		<tbody>
			<tr>
				<td width="50%">
					<h2 style="float:left;"><?php _e('Manage Survey', 'mf-survey'); ?></h2>
				</td>
				<td width="50%">
	...
	<form action="edit.php?page=mf-survey/survey-action.php&amp;noheader=true" method="post" name="form_manage_survey" id="form_manage_survey" autocomplete="off">	
		
		<table class="widefat page fixed" id="tbl_manage_survey" cellspacing="0">
			<thead>
				<tr>
					<th><?php _e('Survey Name', 'mf-survey'); ?></th>
					<th width="6%"><?php _e('Active', 'mf-survey'); ?></th>
					<th></th>
					<th width="5%"></th>
					<th width="6%"></th>
				</tr>
					...
					foreach( $results as $result ) {
						...
						if ( $result->publish_status === 'A' ) {
							...
							if( $count_pages > 0 ) {
								...
								$survey_publish_url = "edit.php?page=mf-survey/survey-action.php&amp;noheader=true?&action=publish_survey&survey_id=".$result->survey_id;
						
								// Publish link
								echo "
									<td style='text-align:right;'>
										<a onclick='publish_survey(\"$survey_publish_url\");' href='javascript:void(0);'>".__('Publish', 'mf-survey')."</a>
									</td>";
								
								// Edit link
								echo "
									<td style='text-align:right;'>
										<a href='admin.php?page=forms/edit-survey-form.php&survey_id=".$survey_id."'>".__('Edit', 'mf-survey')."</a>
									</td>";
									
							} else {
							
								echo '<td style="text-align:right;" colspan="2">'.__('Add page for this survey', 'mf-survey').'</td>';
							
							}
						
							$survey_delete_url = "edit.php?page=mf-survey/survey-action.php&amp;noheader=true?&action=delete_survey&survey_id=".$result->survey_id;
						
							// Delete link
							echo "
								<td style='text-align:right;'>
									<a onclick='delete_survey(\"$survey_delete_url\");' href='javascript:void(0);'>".__('Delete', 'mf-survey')."</a>
								</td></tr>"; 
		...
		<p class="submit">
			<input type="submit" name="btn_manage_survey" id="btn_manage_survey" value="<?php _e('Save', 'mf-survey'); ?>" class="button" />
		</p>
...
<div>
	<a href="admin.php?page=forms/survey-form"><?php _e('Create New Survey', 'mf-survey'); ?></a>
</div>