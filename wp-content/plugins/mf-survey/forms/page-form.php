 ...
 */
wp_register_script( 'page-script', plugins_url() . '/mf-survey/scripts/page-script.js', 'jquery' );
...
if ( isset( $_COOKIE['survey_saved'] ) ) {
	...
	<div id="dv_survey_message" class="updated highlight">
		<p><?php _e('Survey saved', 'mf-survey'); ?></p>
	</div>	
	<?php
...
if ( isset( $_COOKIE['page_available'] ) ) {
	...
	<div id="dv_page_error_message" class="error highlight">
		<p>
			<?php _e('Page name already exits for this survey. Please provide other name.', 'mf-survey'); ?>
		</p>
	</div>	
	<?php
...
<div class="wrap">
	<h2><?php _e('Add Page', 'mf-survey'); ?></h2>
	
	<!-- This form saves page name with survey_id -->
	<form action="edit.php?page=mf-survey/survey-action.php&amp;noheader=true" method="post" name="form_add_page" id="form_add_page" onsubmit="return validate_add_page();" autocomplete="off">	
		...
		<p class="submit">
			<input type="submit" name="btn_submit_page" id="btn_submit_page" value="<?php _e('Save', 'mf-survey'); ?>" class="button" />
		</p>
